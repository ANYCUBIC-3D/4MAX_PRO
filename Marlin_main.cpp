...
static  bool PrintdoneAndPowerOFF=true;
...
void setupLED()
{
 ...
 void Light_CON()
 {
  if(LightConFlag)  WRITE(LED_PIN,HIGH); 
  else   WRITE(LED_PIN,LOW);  
 }
...
void get_command_from_TFT()
{
  ...
  while( NewSerial.available() > 0  && TFTbuflen < TFTBUFSIZE) {        
	...
	//  SERIAL_CHAR(  serial3_char  );//2019.5.5
     if(serial3_char == '\n' ||
      serial3_char == '\r' ||
    ...
    //  if((!planner.movesplanned())&&(!TFTresumingflag))
      ...
      if((!planner.movesplanned())&&(!TFTresumingflag))
       ...
       if((!planner.movesplanned())&&(!TFTresumingflag))             
                         ...
                         {
                          NEW_SERIAL_PROTOCOLPGM("The past value:");
                          NEW_SERIAL_PROTOCOL(MY_Z_PROBE);                              
                           if(TFTcode_seen('S'))
                           {
                             MY_Z_PROBE_OFFSET_FROM_EXTRUDER=TFTcode_value();
                             SaveMyServoAngles(); 
                           }
                         }
                         NEW_SERIAL_SUCC_START;
                         ...
                         */
                         ...
                         if((!planner.movesplanned())&&(!TFTresumingflag))
                         {
                           static float temp_current_positionX=X_MIN_POS,temp_current_positionY=Y_MIN_POS;
                           static float Z_rase_for_safe=10;    
                            enable_z();   
                            enable_x();
                            enable_y();   
                            delay(100);                         
                           ...
                           if(TFTcode_seen('S')) //start leveling
                           {                                           
                             enqueue_and_echo_commands_P(PSTR("G28"));  
                             temp_current_positionX=X_MIN_POS;
                             temp_current_positionY=Y_MIN_POS;
                             PointTestFlag=0;
                           }
                           else if(TFTcode_seen('O'))// FIRST POINT
                           {                                                      
                             PointTestFlag=1;     
                             planner.buffer_line(X_MIN_POS, Y_MIN_POS, Current_z_offset+Z_rase_for_safe, 0, 110, 2);                      
                        //    stepper.synchronize();  
                             temp_current_positionX=10;
                             temp_current_positionY=33;
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset+Z_rase_for_safe, 0, 110, 2);
                   //          stepper.synchronize(); 
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset, 0, 4, 2);
                   //          stepper.synchronize();  
                             enable_x();
                             enable_y();
                             enable_z(); 
                            current_position[X_AXIS]=temp_current_positionX; 
                            current_position[Y_AXIS]=temp_current_positionY; 
                            current_position[Z_AXIS]=Current_z_offset;                            
                           }
                           else if(TFTcode_seen('T')) //second point
                           {                       
                             PointTestFlag=1;                             
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset+Z_rase_for_safe, 0, 110, 2);  
                   //          stepper.synchronize();                                                         
                             temp_current_positionX=207;
                             temp_current_positionY=33;
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset+Z_rase_for_safe, 0, 110, 2);
                     //        stepper.synchronize(); 
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset, 0, 4, 2);
                      //       stepper.synchronize();
                           ...
                           else if(TFTcode_seen('C')) // thrid point
                           {                             
                             PointTestFlag=1;
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset+Z_rase_for_safe, 0, 110, 2);                             
                      //       st_synchronize();
                             temp_current_positionX=207;
                         //    temp_current_positionY=210;
                             temp_current_positionY=223;
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset+Z_rase_for_safe, 0, 110, 2);
                             stepper.synchronize(); 
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset, 0, 4, 2);
                            stepper.synchronize();
                           ...
                           else if(TFTcode_seen('Q')) // thrid point
                           {                             
                             PointTestFlag=1;
                             planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset+Z_rase_for_safe, 0, 110, 2);                             
                  //          stepper.synchronize();
                         ...
                         //    temp_current_positionY=210;
                           ...
                           else if(TFTcode_seen('H')||TFTcode_seen('L'))
                           {
                             PointTestFlag=0;
                        //     Z_offset_debug_flag=0;
                      //       planner.buffer_line(temp_current_positionX, temp_current_positionY, Current_z_offset+Z_rase_for_safe, 0, 100, 2);  
                       //      stepper.synchronize();
                             if(TFTcode_seen('L'))
                             {                                                                                              
                               relative_mode = false;// G90 
                               AutoLevelLowSpeedModelFlag=1;                             
                             }
                                AssistLeveTestflag=1;
                           }
                         }
                       //  NEW_SERIAL_SUCC_START;
                         TFT_SERIAL_ENTER();
      ...
      if((!planner.movesplanned())&&(!TFTresumingflag))
                         {
                          ...
                          if((READ(Z_TEST)==0)) z_offset_auto_test();
                      //    Z_offset_debug_flag=1;
                          if(TFTcode_seen('S')){
                          NEW_SERIAL_PROTOCOLPGM("A9V ");
                          NEW_SERIAL_PROTOCOL(int(Current_z_offset*100)); 
                          TFT_SERIAL_ENTER();
...
void setupMyZoffset()
{
  ReadMyZoffset();
  readFirstBootFlag();   
  if(FirstBootFlag!=0x5a) {
  FirstBootFlag=0x5a;
  SaveFirstBootFlag();
  Current_z_offset=0.2;
  SaveMyZoffset();}
  else Current_z_offset=last_z_offset;
}
...
void Fan2Scan()
{
  if(PrintdoneAndPowerOFF&&powerOFFflag&&(thermalManager.degHotend(0)<100)){powerOFFflag=0;PowerDown();}
  if(thermalManager.degHotend(0)>65) WRITE(V5_COOLING_PIN, HIGH); 
...
if(Scancount>16000){ Scancount=0;NEW_SERIAL_PROTOCOLPGM("J10");TFT_SERIAL_ENTER();}//T0 unnormal//2019.4.23
}
...
void AssistLevelTest()
{
  ...
  enqueue_and_echo_commands_P(PSTR("G28"));  
  stepFlag=1;
 if(AutoLevelLowSpeedModelFlag)  enqueue_and_echo_commands_P(PSTR("M109 S200"));// EXTRADER 0  200degree   
  }
  if(HomeFlag){
  stepper.synchronize();
  feedrate_percentage =1500.0;  
  if(AutoLevelLowSpeedModelFlag&&(thermalManager.degTargetHotend(0)>=198)){   
  planner.buffer_line(X_MIN_POS, 0, 5, 3, feedrate_percentage/60, 0);
  planner.buffer_line(20, 20, 0.2, 10, feedrate_percentage/60, 0);
  planner.buffer_line(20, 190, 0.2, 17, feedrate_percentage/60, 0);
  planner.buffer_line(190, 190, 0.2, 24, feedrate_percentage/60, 0);
  planner.buffer_line(190, 20, 0.2, 31, feedrate_percentage/60, 0);
  planner.buffer_line(20, 20, 0.2, 39, feedrate_percentage/60, 0);
  planner.buffer_line(20, 20, 10, 40, feedrate_percentage/60, 0);
  current_position[X_AXIS]=20; 
  current_position[Y_AXIS]=20; 
  current_position[Z_AXIS]=10; 
  planner.set_e_position_mm(0);
  stepper.synchronize();
  AutoLevelLowSpeedModelFlag=0;    
  ...
  else if(!AutoLevelLowSpeedModelFlag){  
   stepper.synchronize();   
  planner.buffer_line(X_MIN_POS, 0, 5, 0, feedrate_percentage/60, 2);
  stepper.synchronize();
  planner.buffer_line(20, 20, 0.2, 0, feedrate_percentage/60, 2);
  planner.buffer_line(20, 190, 0.2, 0, feedrate_percentage/60, 2);
  planner.buffer_line(190, 190, 0.2, 0, feedrate_percentage/60, 2);
  planner.buffer_line(190, 20, 0.2, 0, feedrate_percentage/60, 2); 
  planner.buffer_line(20, 20, 0.2, 0, feedrate_percentage/60, 2);
  planner.buffer_line(20, 20, 10, 0, feedrate_percentage/60, 2); 
  current_position[X_AXIS]=20; 
  current_position[Y_AXIS]=20; 
  current_position[Z_AXIS]=10; 
  stepper.synchronize();  
